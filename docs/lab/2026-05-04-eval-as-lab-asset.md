...
測試的基礎文件是《金融機構辦理電子銀行業務安全控管作業基準》（115 年 1 月 7 日版），這是台灣金融機構辦理電子銀行業務的核心監理依據，每一條都有明確的法律效力。
...
這不是隨機出題。每一道題背後都有明確的評分依據，而且題目本身帶有工作現場的溫度——這些問題，就是我在工作裡真正需要回答的問題。
...
金融法規的理解，需要的是真正具備法律和合規背景的人來判斷。一個模型說它引用了「第六條第三項」，但這個引用是否精確、是否用對了地方——這件事，需要懂這份文件的人來確認，不是另一個 AI。
...
從 Infra 主管的角度來看，我建這套系統，不只是為了測哪個模型比較好。

它真正回答的問題是：**在金融機構導入本地 LLM 之前，你要用什麼方式評估它是否適任？**

現在很多人在討論企業 AI 部署，都停留在「這個模型速度很快」、「這個模型中文很流暢」的層次。但如果你要把模型放進法遵流程、內部稽核、客服問答，你需要的評估維度完全不同。

這套雙軌評分系統，是一個起點：

- **題組設計**對應真實工作場景，不是泛用 benchmark
- **AI 自動評分**提供可重複的基準
- **人工評分**引入領域知識的驗證
...
人工評分的部分，明天請合規領域的專家來填。填完之後，我會更新這篇文章，加上 AI vs 人工的差異分析。

這套方法論，我打算繼續完善。下一個版本想加入的功能：讓不同模型回答同一道情境題，然後讓評審不知道哪個回答是哪個模型的，做盲測比較。
